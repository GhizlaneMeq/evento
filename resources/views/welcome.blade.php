...
@endsection
 ...
 @else
 @foreach($events as $event)
 <div class="events flex-col p-8 py-16 rounded-lg shadow-2xl md:p-12 bg-gradient-to-b from-gray-900 to-black event-item"
     onclick="showEventModal('{{ $event->title }}', '{{ $event->description }}', '{{ $event->category->name }}')">
     <a class="relative" href="{{ route('events.show', $event->id) }}">
         @if ($event->image)
         <img class="rounded relative w-full object-cover aspect-video" src="{{ asset('storage/' . $event->image) }}"
             alt="{{ $event->title }}" loading="lazy">
         @else
         <img class="rounded relative w-full object-cover aspect-video" src="https://via.placeholder.com/800x400?text=No+Image"
             alt="{{ $event->title }}" loading="lazy">
         @endif
     </a>
     <div class="flex ">
         <img alt="" src="" class="object-cover w-12 h-12 rounded-full shadow dark:bg-gray-500">
         <div class="flex flex-col">
             <a rel="noopener noreferrer" href="#"
                 class="text-sm font-semibold">{{ $event->organizer->user->name }}</a>
             <span class="text-xs dark:text-gray-400">{{ $event->created_at->diffForHumans() }}</span>
         </div>
     </div>
 ...
 <script>
     function showEventModal(title, description, category) {
                 ...
                 .then(data => {
                     ...
                     data.events.forEach(event => {
                         ...
                         <div class="events flex-col p-8 py-16 rounded-lg shadow-2xl md:p-12 bg-gradient-to-b from-gray-900 to-black event-item"
                             onclick="showEventModal('${event.title}', '${event.description}', '${event.category.name}')">
                             <a class="relative" href="{{ route('events.show', $event->id) }}">
                                 ${event.image ? `<img class="rounded relative w-full object-cover aspect-video" src="{{ asset('storage/' . $event->image) }}" alt="${event.title}" loading="lazy">` :
                                 `<img class="rounded relative w-full object-cover aspect-video" src="https://via.placeholder.com/800x400?text=No+Image" alt="${event.title}" loading="lazy">`}
                             </a>
                             <div class="flex ">
                                 <img alt="" src="" class="object-cover w-12 h-12 rounded-full shadow dark:bg-gray-500">
                                 <div class="flex flex-col">
                             ...
                             <div class="flex flex-wrap justify-between">
                                 <!-- Reservation and other details here -->
                             </div>
                         </div>
                         `;
                         eventsList.insertAdjacentHTML('beforeend', eventHtml);
