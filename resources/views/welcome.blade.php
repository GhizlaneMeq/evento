...
@endsection
 ...
 @section('events')
     ...
     @else
         @foreach($events as $event)
             <div class="flex-col p-8 py-16 rounded-lg shadow-2xl md:p-12 bg-gradient-to-b from-gray-900 to-black"   class="event-item" onclick="showEventModal('{{ $event->title }}', '{{ $event->description }}', '{{ $event->category->name }}')">
                 <a class="relative" href="{{ route('events.show', $event->id) }}">
                     @if ($event->getMedia('event_images')->isNotEmpty())
                         <img class="rounded relative w-full object-cover aspect-video" src="{{ $event->getFirstMediaUrl('event_images') }}" alt="{{ $event->title }}" loading="lazy">
                     @else
                 ...
                 <div class="flex flex-col justify-beetween gap-3 px-4 py-2">
                     <a href="{{ route('events.show', $event->id) }}" class="flex justify-center items-center text-xl font-semibold text-teal-700 hover:text-teal-800 two-lines text-ellipsis">
                         <span>{{ $event->title }}</span>
                     </a>
                     <span>{{ $event->category->name }}</span>
                 </div>
