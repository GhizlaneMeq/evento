...
@section('content')
<div class="mt-4">
    <h1 class="sr-only">Create New Event</h1>
    <div class="max-w-lg mx-auto">
        <form action="{{ route('organizer.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4" >
            @csrf
            <div class="mb-4">
                <label for="title" class="block text-gray-700 text-sm font-bold mb-2">Title</label>
                <input type="text" name="title" id="title" class="form-input w-full @error('title') border-red-500 @enderror" value="{{ old('title') }}">
                @error('title')
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date" id="date" class="form-input w-full @error('date') border-red-500 @enderror" value="{{ old('date') }}">
                @error('date')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="location" id="location" class="form-input w-full @error('location') border-red-500 @enderror" value="{{ old('location') }}">
                @error('location')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="number" name="availableSeats" id="availableSeats" class="form-input w-full @error('availableSeats') border-red-500 @enderror" value="{{ old('availableSeats') }}">
            ...
            <div class="mb-4">
                ...
                <div class="flex items-center space-x-4">
                    <label class="inline-flex items-center">
                        <input type="radio" class="form-radio" name="reservationMethod" value=0 {{ old('reservationMethod') == '0' ? 'checked' : '' }}>
                        <span class="ml-2">Automatic</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" class="form-radio" name="reservationMethod" value=1 {{ old('reservationMethod') == '1' ? 'checked' : '' }}>
